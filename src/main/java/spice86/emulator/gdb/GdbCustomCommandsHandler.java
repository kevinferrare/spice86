...
import spice86.emulator.function.FunctionHandlerDumper;
...
public class GdbCustomCommandsHandler {
    ...
    return gdbIo.generateMessageToDisplayResponse(additionalMessage +
        """
            Supported custom commands:
             ...
             - dumpFunctions <file path to dump>: dump information about the function calls executed
  ...
  private String dumpFunctions(String[] args) {
    String fileName = getFirstArgumentOrDefaultFile(args, "Functions.txt");
    return doFileAction(fileName, f -> {
      Cpu cpu = machine.getCpu();
      new FunctionHandlerDumper().dump(f, cpu.getFunctionHandler(),
          cpu.getFunctionHandlerInExternalInterrupt());
    }, "Error while dumping functions");
  }

  private String dumpJavaStubs(String[] args) {
    String fileName = getFirstArgumentOrDefaultFile(args, "JavaStubs.txt");
    return doFileAction(fileName, f -> {
      Cpu cpu = machine.getCpu();
      new FunctionHandlerDumper().dumpJavaStubs(f, cpu.getFunctionHandler(),
          cpu.getFunctionHandlerInExternalInterrupt());
    }, "Error while dumping function stubs");
  }
  ...
  private String getFirstArgumentOrDefaultFile(String[] args, String suffix) {
    if (args.length >= 2) {
      return args[1];
    }
    return "spice86dump" + suffix;
  }
}
